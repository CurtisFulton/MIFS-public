...
namespace Mifs.Scheduling
{
            ...
            where TEntityProcessor : class, IProcessEntity<TEntity>
    {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            if (this.DataProvider is IJobInitializer dataProviderInitializer)
            {
                await dataProviderInitializer.Initialize(cancellationToken);
            }

            if (this.DataProvider != this.EntityProcessor && this.EntityProcessor is IJobInitializer entityProcessorInitializer)
            {
